...
describe('GET /api/v1/schools/:uuid/risk', () => {

  it('should return the school risk score for User', async () => {
    const app = buildAuthTestApp();
    const response = await request(app).get(`/api/v1/schools/${process.env.SCHOOL_ONE_UUID}/risk`)
      .set('Authorization', `Bearer ${process.env.USER_1_TOKEN}`);

    expect(response.status).to.equal(200);
    expect(response.body.status).to.equal(true);
    expect(response.body.data.latest).to.have.property('school_id');
    ...
    const app = buildAuthTestApp();
    const response = await request(app).get(`/api/v1/schools/${process.env.SCHOOL_TWO_UUID}/risk`)
  ...
  it('should return 401 if invalid token provided', async () => {
    ...
    const res = await request(app)
      .get(`/api/v1/schools/${process.env.SCHOOL_TWO_UUID}/risk`)
  ...
  it('should return 404 for non-existent school UUID', async () => {
    ...
    const res = await request(app)
      .get(`/api/v1/schools/${fakeUuid}/risk`)
      .set('Authorization', `Bearer ${process.env.ADMIN_1_TOKEN}`);
